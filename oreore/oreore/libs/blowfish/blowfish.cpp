...
namespace oreore
{
    namespace
    {
        ...
        unsigned long gcd(const unsigned long larger, const unsigned long smaller)
        {
    ...
    void Blowfish::setKey(const char *key, const size_t byte_length)
    {
        ...
        const unsigned long buffer_length = byte_length / gcd(byte_length, sizeof(uint32_t));
        std::vector<uint32_t> buffer(buffer_length);
        ...
        for(unsigned long i = 0; i < buffer_length; ++i)
        {
            ...
            buffer[i] = converter.bit_32;
        }

        for (int i = 0; i < pary_length; ++i)
        {
            uint32_t key_uint32 = buffer[i % buffer_length];
            pary_[i] ^= key_uint32;
        }
    ...
    uint32_t Blowfish::feistel(const uint32_t value) const
    {
        Converter32 converter;
        converter.bit_32 = value;

        const uint8_t a = converter.bit_8.byte0;
        const uint8_t b = converter.bit_8.byte1;
        const uint8_t c = converter.bit_8.byte2;
